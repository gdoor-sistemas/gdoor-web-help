...
Listamos neste artigo como utilizar o código interno dos produtos como código de barras, incluindo a configuração da replicação para novos produtos e produtos já cadastrados, além da definição do código principal.
...
O recurso pode ser configurado para replicar o código interno apenas nos novos produtos ou também nos produtos já cadastrados. Para os produtos existentes, também é possível definir se o código replicado será utilizado como código de barras principal.
...
> Um produto pode possuir mais de um código de barras cadastrado. Quando houver vários códigos, um deles poderá ser definido como principal. {.is-info}
...
> Os produtos já existentes não serão alterados. A partir dessa configuração, os novos cadastros poderão receber o código interno também como código de barras. {.is-warning}
...
Para interromper a replicação automática nos novos produtos, acesse **"Configurações"**, selecione **"Geral"** e desmarque a opção **"Replicar código interno no código de barras"**.
